...
void statStruct::printInfo(){
	...
	cout<<"integrality gap  : "<< optimalityGap << endl;
...
void statStruct::writeFile(string s){
	...
	fichier <<formulation << ";"<<totalTime<<";"<<nbNodes<<";"<<nbCuts[0]<<";" << temps[0]<< ";" << nbCuts[2] <<";" <<  temps[2]<< ";" << nbCuts[3]<< ";"  << temps[3] << ";" << nbCuts[4]<< ";"  << temps[4] << ";"<<nbCst<<";" << nbNoeudInstance << ";" << nbAreteInstance << ";"<< name << ";;"<< endl;
	fichier.close();
